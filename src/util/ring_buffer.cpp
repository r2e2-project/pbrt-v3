...
RingBuffer::RingBuffer(const size_t capacity)
    : fd_([&] {
      ...
      first_mapping_(virtual_address_space_.addr(), capacity,
                     PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED,
                     fd_.fd_num()),
      second_mapping_(virtual_address_space_.addr() + capacity, capacity,
                      PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED,
                      fd_.fd_num()) {}
...
void RingBuffer::push(const size_t num_bytes) {
    if (num_bytes > writable_region().length()) {
        throw runtime_error(
            "RingBuffer::wrote exceeded size of writable region");
    }
